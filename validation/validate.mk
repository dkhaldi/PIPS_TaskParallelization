# $Id: validate.mk 6829 2016-10-30 08:02:23Z coelho $
#
...
# Trying to be helpful... if other versions of ipyps a gfc2pips are
# found in the path, they will be used even if they belong to another pips.
#
...
FLT	= sed -e 's,$(here)/\./,$$VDIR/,g;s,$(here),$$VDIR,g'

# where to store validation results
# this is the default, but may need to be overriden
RESULTS	= RESULTS

# this macro allows to control how results are displayed
#TORESULTS = >> $(RESULTS)
...
PF	= \
	@echo "processing $(SUBDIR)/$+" ; \
	  ...
	  export PIPS_MORE=cat PIPS_TIMEOUT=$(TIMEOUT) \
	    LC_ALL=C PIPS_VALIDATION_IN_PROGRESS=1 ; \
...
%.rec: %
	...
	if [ -f "$^/Makefile" ] ; then 					\
	  $(MAKE) RECWHAT=$$recwhat RESULTS=../$(RESULTS) 		\
	    SUBDIR=$(SUBDIR)/$^ -C $^ $(FORWARD) || 			\
	  echo "broken-directory: $(SUBDIR)/$^" $(TORESULTS) ; 		\
	else 								\
	  echo "missing-makefile: $(SUBDIR)/$^" $(TORESULTS) ; 		\
	fi
...
clean-validate:
	$(RM) *~ *.o *.s *.tmp *.err *.diff *.result/out out err a.out RESULTS SUMMARY
	$(RM) -r *.database .PYPS*.tmp .*.tmp

.PHONY: rec-clean
rec-clean:
	[ "$(D.rec)" ] && \
	  $(MAKE) FORWARD=clean RESULTS=/dev/null $(D.rec) || exit 0
...
validate:
	...
	# run "make {later,bug,slow,default}-validate-{test,out}"
	#      for testing subsets...
...
.PHONY: unvalidate
unvalidate: MAKEFLAGS += -j1
unvalidate: # recursion to activate -j1
	$(MAKE) do-unvalidate rec-unvalidate

# BUG: should check that $(F.output) is not empty
.PHONY: do-unvalidate
...
%.validate: %.c $(DEFTEST)
	$(PF) ; WSPACE=$* FILE=$(here)/$< VDIR=$(here) ./$(DEFTEST) \
	2> $*.err | $(FLT) > $*.result/$(TEST) ; $(OK)
...
# check that test cases use a database of the test case name
.PHONY: dbname
dbname:
	pips_validation_check_database.pl \
		$(F.tpips) $(F.tpips2) $(F.test) $(F.test) \
		$(TORESULTS)

# all possible inconsistencies
.PHONY: inconsistencies
inconsistencies: skipped orphan multi-source multi-script nofilter empty-test dbname
