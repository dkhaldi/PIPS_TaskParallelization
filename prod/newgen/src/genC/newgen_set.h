/*
  ...
  $Id: newgen_set.h 1357 2016-03-02 08:18:50Z coelho $
...
#define SET_FOREACH(type_name, the_item, the_set)                       \
  hash_table NGMID(h) = set_private_get_hash_table(the_set);            \
  void * NGMID(i) = NULL;                                               \
  for (type_name the_item;                                              \
       (NGMID(i) =                                                      \
        hash_table_scan(NGMID(h), NGMID(i), (void **) &the_item, NULL));)
