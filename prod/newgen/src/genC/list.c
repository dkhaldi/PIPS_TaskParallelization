/*
  ...
  $Id: list.c 1365 2016-09-24 06:08:13Z coelho $
...
struct __gen_sweep_state {
  bool done;                       // no more updates are possible
  gen_sweep_direction dir;         // forward or backward
  list initial, * phead,  * ptail; // sweep parameters
  list last, head, tail, reversed; // internal helpers
};

/* initialize list l sweep in direction dir, with pointers for head & tail
 */
gen_sweep_state
gen_sweep_init(list l, gen_sweep_direction dir, list * phead, list * ptail)
{
  gen_sweep_state state = malloc(sizeof(struct __gen_sweep_state));
  message_assert("malloc ok", state != NULL);
  ...
  switch (dir)
  {
  case gen_sweep_forward:
    state->head = NIL;
    state->tail = l;
    state->last = NIL;
    state->reversed = NIL; // unused
    break;

  case gen_sweep_backward:
    state->head = l;
    state->tail = NIL;
    state->last = NIL; // unused
  {
    list nl = NIL, c = l;
    while (c != NIL) {
      nl = CONS(LIST, c, nl);
  ...
  default:
    fprintf(stderr, "unexpected sweep direction: %d\n", dir);
    abort();
  }

  // update external pointers
  if (state->phead)
    *(state->phead) = state->head;
  if (state->ptail)
    *(state->ptail) = state->tail;
...
bool gen_sweep_update(gen_sweep_state state)
{
  ...
  case gen_sweep_forward:
    // first update
    if (state->head == NIL)
      state->head = state->last = state->tail;
    else
      // relink last cons
      CDR(state->last) = state->tail;
    // set new last cons
    state->last = state->tail;
    // move forward tail
    state->tail = CDR(state->tail);
    // check for end
    state->done = state->tail == NIL;
    // cut last const
    if (!state->done)
      CDR(state->last) = NIL;
    break;
  ...
  case gen_sweep_backward:
  {
    list previous_tail = state->tail, tmp = state->reversed;
    // tails move backwards
    state->tail = LIST(CAR(state->reversed));
    // relink tail
    CDR(state->tail) = previous_tail;
    // move backwards
    state->reversed = CDR(state->reversed);
    // cleanup
    free(tmp);
    // last one
    if (state->reversed == NIL)
      state->head = NIL;
    // check for end
    state->done = state->head == NIL;
  }
    break;
...
{
  message_assert("some state", state != NULL);

  // complete sweep to relink the initial list
  state->phead = NULL;
  state->ptail = NULL;
  while (gen_sweep_update(state));
