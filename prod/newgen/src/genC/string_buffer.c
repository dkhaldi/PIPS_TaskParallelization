/*
  ...
  $Id: string_buffer.c 1357 2016-03-02 08:18:50Z coelho $
 ...
 * including external double-quotes. It adds escapes for special characters.
 */
...
{
  if (*s)
    string_buffer_append_internal(sb, sb->dup? strdup(s): s);
}
 ...
 */
void string_buffer_printf_alist(
  string_buffer sb,
  const string format,
  va_list *args)
{
  ...
  va_copy(acpy, *args);
  string str;
  int err = vasprintf(&str, format, acpy);
  ...
  ...)
{
  ...
  string_buffer_printf_alist(sb, format, &args);
  va_end(args);
}

/* append a string with XML escapes
 */
void string_buffer_append_xml_text(
  string_buffer sb,
  const string stuff,
  bool also_nl)
{
  // count needed chars, with a margin
  int len = 1; // final '\0'
  char * s = stuff;
  while (*s) {
      // only five mandatory ("'&<>), \n added...
  ...
  // allocate
  char * escaped = (char*) malloc(sizeof(char)*len);

  // copy stuff & insert escapes
  s = stuff;
  char * e = escaped;
  while (*s) {
    switch (*s) {
      // mandatory
    case '"':
      *e++='&'; *e++='q'; *e++='u'; *e++='o'; *e++='t'; *e++=';'; break;
    case '\'':
      *e++='&'; *e++='a'; *e++='p'; *e++='o'; *e++='s'; *e++=';'; break;
    case '&':
      *e++='&'; *e++='a'; *e++='m'; *e++='p'; *e++=';'; break;
    case '<':
      *e++='&'; *e++='l'; *e++='t'; *e++=';'; break;
    case '>':
      *e++='&'; *e++='g'; *e++='t'; *e++=';'; break;
  ...
  *e = '\0';
  ...
  string_buffer_append_internal(sb, escaped);
}
