/*
  ...
  $Id: text_print.c 23065 2016-03-02 09:05:50Z coelho $
...
void print_sentence(FILE * fd, sentence s) {
  ...
  if (sentence_formatted_p(s)) {
    ...
    FOREACH(string, w, lw) {
      switch(lang) {
        ...
        case is_language_fortran95: {
          ...
          if (col + (int) strlen(w) <= max_line_size - 2) {
           ...
           * to avoid line overflow (FI, March 1993) */
            ...
            ncar = MIN(max_line_size - col + 1, (int) strlen(line));
            ;
            ...
            while(strlen(line) != 0) {
              ncar = MIN(max_line_size - 7 + 1, (int) strlen(line));
    ...
    /* Output the statement line number on the right end of the
     line: */
    if (n > 0 && get_bool_property("PRETTYPRINT_STATEMENT_NUMBER")) {
      for (int i = col; i <= column_start; i++) {
        putc_sentence(' ', fd);
      }
      switch (get_prettyprint_language_tag()) {
        case is_language_fortran:
 ...
 * BUG (FC): words_to_string is malloced, but sentence_formatted is not:-(
 */
...
string text_to_string_gen(text t, bool with_newline)
{
  ...
  FOREACH(sentence, sen, text_sentences(t))
  {
    string ss = sentence_to_string(sen);
    string_buffer_append(sb, ss);
    // try to avoid a memory leak... see comments above
    if (sentence_unformatted_p(sen))
      free(ss);
    if (with_newline)
      string_buffer_append(sb, "\n");
  }

  string s = string_buffer_to_string(sb);
...
{
  return text_to_string_gen(t, false);
}

string text_to_string_nl(text t)
{
  return text_to_string_gen(t, true);
}
