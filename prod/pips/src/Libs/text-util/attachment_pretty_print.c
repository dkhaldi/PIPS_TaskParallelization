/*
  ...
  $Id: attachment_pretty_print.c 23065 2016-03-02 09:05:50Z coelho $
...
#include "constants.h" // *_EXT macro
 ...
 * FC: DUPLICATED FROM ri-util TO AVOID A DEPENDENCY
 */
...
void
begin_attachment_prettyprint(void)
{
...
void
end_attachment_prettyprint(void)
{
...
attach_to_character_region(
  ...
  attachee at)
{
  // We do not know the position of the attachement in the output file yet:
  attachment a = make_attachment(at, POSITION_UNDEFINED, POSITION_UNDEFINED);
...
/* Attach a loop: */
void
attach_loop_to_sentence_up_to_end_of_text(sentence s, text t, loop l)
{
  if (is_emacs_pretty_print_asked)
...
sentence
attach_head_to_sentence(sentence s,	entity module)
{
  if (is_emacs_pretty_print_asked)
