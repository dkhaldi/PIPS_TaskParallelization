/*
  ...
  $Id: freia_opencl.c 23288 2016-11-08 10:09:39Z coelho $
...
#define VARSHIFT(v,n) (((char*)v)+(n))
  ...
  if (is_input && !set_belong_p(loaded, VARSHIFT(v, shft)))
  {
    if (get_bool_property("HWAC_OPENCL_PRELOAD_PIXELS"))
    {
      ...
      sb_cat(load, "j", i2a(in), "[i", shift[shft+4], "];\n");
...
{
  string cut_name = strdup(cat(split_name, "_", i2a(n_cut)));
  ...
  FOREACH(dagvtx, v, dag_outputs(d))
  {
    ...
    // p<out>[i] = t<n>;
    sb_cat(opencl_tail,
         BODY, "p", si, "[i] = t", i2a((int) dagvtx_number(v)), ";\n");
    cl_args+=2;
    nargs++;
    free(si);
    i++;
  ...
  FOREACH(dagvtx, v, vertices)
  {
    ...
    string svn = strdup(i2a((int) dagvtx_number(v)));
...
#define RED " = redres." // for a small code compaction
      ...
      else if (same_string_p(api->compact_name, "max!"))
      {
        ...
        sb_cat(helper_tail, "  *po", i2a(nargs-3), RED "maximum;\n");
        sb_cat(helper_tail, "  *po", i2a(nargs-2), RED "max_coord_x;\n");
        sb_cat(helper_tail, "  *po", i2a(nargs-1), RED "max_coord_y;\n");
      }
      else if (same_string_p(api->compact_name, "min!"))
      {
        sb_cat(opencl_end,
        ...
        sb_cat(helper_tail, "  *po", i2a(nargs-3), RED "minimum;\n");
        sb_cat(helper_tail, "  *po", i2a(nargs-2), RED "min_coord_x;\n");
        sb_cat(helper_tail, "  *po", i2a(nargs-1), RED "min_coord_y;\n");
      }
    }
    else if (is_a_kernel)
    {
  ...
  sb_cat(helper, ", ", i2a(n_outs));   // output images
  sb_cat(helper, ", ", i2a(n_ins));    // input images
  sb_cat(helper, ", ", i2a(n_params)); // input integer parameters
  sb_cat(helper, ", ", i2a(n_misc));   // output integer pointers
  ...
  FILE * helper_file, FILE * opencl_file, set helpers)
{
  ...
  string func_name = strdup(cat(prefix, "_", i2a(number)));
...
static void opencl_merge_and_compile(
  ...
  hash_table signatures)
{
  string split_name = strdup(cat(fname_fulldag, "_", i2a(n_split)));
