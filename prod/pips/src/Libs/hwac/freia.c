/*
  ...
  $Id: freia.c 23284 2016-11-05 16:46:14Z coelho $
...
/* transpose a kernel expression
 */
expression freia_transpose_kernel(const expression kref) {
  // get the kernel array
  list vals = call_arguments(syntax_call(expression_syntax(kref)));
  entity fref = call_function(syntax_call(expression_syntax(kref)));
  
  // reverse it...
  list vals_rev = gen_nreverse(gen_full_copy_list(vals));

  // build the expression
  return make_expression(make_syntax_call(make_call(fref, vals_rev)),
			 expression_normalized(kref));
} 

/* detect a call to a kernel transposition function
 * and transpose the arguments
...
bool freia_is_transpose_call(statement s){
  if(statement_call_p(s)) {
	...
	list args = call_arguments(c);
	pips_assert("an expected transposition function should have at least two arguments",
		    gen_length(args)>=2);

	// get the argument entities
	entity kernelTrp = expression_to_entity(EXPRESSION(CAR(args)));
	entity kernelRef = expression_to_entity(EXPRESSION(CAR(CDR(args))));

	if(value_expression_p(entity_initial(kernelRef))) {
	  // get the expression value of the initial kernel
	  expression kernelRefValExpr = value_expression(entity_initial(kernelRef));
	  // effectively transpose kernel
	  expression kernelTrpValExpr = freia_transpose_kernel(kernelRefValExpr);

	  // set the value of the transposed kernel
	  set_entity_initial(kernelTrp, kernelTrpValExpr);

	  pips_debug(9, "%s: out %s, in %s\n",
		     entity_local_name(called),
		     expression_to_string(kernelTrpValExpr),
		     expression_to_string(kernelRefValExpr));
	}
