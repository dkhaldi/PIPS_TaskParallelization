/*
  ...
  $Id: type.c 23120 2016-06-29 16:30:17Z irigoin $
...
/* Field entities do not have the same names when the derived type
 * declarations are compiled in different C files, even when they are
 * located in the same header file.
 *
 * Filed entities cannot be declared as global to avoid name
 * conflicts.
 *
 * FI: I wonder if this is taen into account in type_equal_p()...
 */
entity find_field_in_field_list(entity f, list fl) {
  ...
  if(entity_is_argument_p(f, fl))
    ...
    const char * n = entity_user_name(f);
    FOREACH(ENTITY, e, fl) {
      if(strcmp(entity_user_name(e), n)==0)
	/* FI: Not 100% bullet proof! */
	nf = e;
    }
  }
  return nf;
...
static bool reference_dependence_variable_check_and_add(reference ref, list *dependence_list) {
  entity var = reference_variable(ref);
  if (!const_variable_p(var)) {
    /* Add the dependence reference only if not already here: */
    if (gen_chunk_undefined_p(gen_find(ref, *dependence_list, (gen_filter2_func_t) reference_equal_p, (void * (*)(const union gen_chunk)) gen_identity)))
...
list dependence_of_dependent_type(type t)
{
  ...
  if(type_variable_p(t)) {
    ...
    FOREACH(DIMENSION, d, dl) {
      ...
      //if(!extended_expression_constant_p(l))
        gen_context_recurse(l, &dep, reference_domain, gen_true, reference_dependence_variable_check_and_add);

      //The test is redundant with the gen_recurse
      //if(!extended_expression_constant_p(u))
        gen_context_recurse(u, &dep, reference_domain, gen_true, reference_dependence_variable_check_and_add);
    }
