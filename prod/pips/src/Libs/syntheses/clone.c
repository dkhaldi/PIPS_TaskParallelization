/*
  ...
  $Id: clone.c 23065 2016-03-02 09:05:50Z coelho $
...
#include "phases.h"
#if defined(BUILDER_CLONE) ||                   \
  defined(BUILDER_CLONE_ONLY) ||                \
  defined(BUILDER_CLONE_ON_ARGUMENT) ||         \
  defined(BUILDER_CLONE_SUBSTITUTE)
...
#include "prettyprint.h" // for text_named_module
#include "effects-util.h"

#include "pipsmake.h" // add_new_module_from_text
#include "preprocessor.h" // compilation_unit_of_module
...
static void clone_error_handler()
{
    ...
    entity substitute  /* entity to substitute to module */)
{
  ...
  reset_hooks_register(clone_error_handler);
  ...
  /* close
   */
  reset_hooks_unregister(clone_error_handler);
  module_to_clone = entity_undefined;
  argument_to_clone = 0;
  statement_to_clone = STATEMENT_NUMBER_UNDEFINED;
  clonee_to_substitute = entity_undefined;
 ...
 *       < CALLERS.code
 */
bool clone_on_argument(const string name)
{
    entity module;
 ...
 */
bool clone_substitute(const string name)
{
...
bool clone_only(const string mod_name)
{
    /* get the resources */
