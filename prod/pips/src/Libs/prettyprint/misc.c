/*
  ...
  $Id: misc.c 23181 2016-09-09 12:56:48Z irigoin $
...
char lib_ri_util_prettyprint_c_rcsid[] = "$Id: misc.c 23181 2016-09-09 12:56:48Z irigoin $";
  ...
  *
  * FI: I am changing the semantics of pdl and record the entities
  * that have already been declared, possibly in previous
  * statements. To update this list, a pointer to pdl, ppdl, is now
  * passed.
  *
  ...
  *        generic_text_statement_enclosed: to manage recursivity
  *           (proper statement or compound statement)
...
words_io_inst(call obj,
	      int precedence, bool leftmost, list * ppdl)
{
  list pc = NIL;
...
static list
words_unary_minus(call obj, int precedence, bool leftmost, list * ppdl)
{
    list pc = NIL;
    expression e = EXPRESSION(CAR(call_arguments(obj)));
    int prec = words_intrinsic_precedence(obj);
 ...
 */
list /* of string */
eole_fma_specific_op(call obj, int precedence, bool leftmost, list * ppdl)
{
...
list /* of string */
eole_fms_specific_op(call obj, int precedence, bool leftmost, list * ppdl)
{
...
static list /* of string */
words_infix_nary_op(call obj, int precedence, bool leftmost, list * ppdl)
{
  list /*of string*/ pc = NIL;
  list /* of expressions */ args = call_arguments(obj);
 ...
 */
static list
words_infix_binary_op(call obj, int precedence, bool leftmost, list * ppdl)
{
  list pc = NIL;
  list args = call_arguments(obj);
  int prec = words_intrinsic_precedence(obj);
...
static list
words_intrinsic_call(call obj, int precedence, bool leftmost, list * ppdl)
{
    if(hash_table_undefined_p(intrinsic_handlers)) {
        init_intrinsic_handlers();
    }
...
/* This one is exported. Outer parentheses are never useful. ppdl can
   point to an empty list, but it must be free on return*/
list /* of string */ words_expression(expression obj, list * ppdl)
{
 ...
 */
static list /* of string */
loop_private_variables(loop obj, list * ppdl)
{
 ...
 *
 * Braces are now managed by text_block_ifthen, text_block_elseif or
 * text_block_else to factorize the work and facilitate the engineering.
 * A difference on the output code can be to not put "}" and else
 * in the same line?
 */
 ...
 *
 * Note: this is supposed to have been dealt with by another function
 * called before, ensure_comment_consistency()
 */
...
/* return a formatted comment, that takes care of adding the relevant // or C
 * depending on output language, except for empty lines in C
 */
...
/* Build the text of a statement
   ...
   @param ppdl: pointer to the previous declaration list; list of
   entities that have already been declared and should not be
   redeclared; this is required for struct and union which may be
   declared independently or in a nested way. See C_syntax/struct03,
   04, 05, etc...
...
*/
text generic_text_statement_enclosed(
  entity module,
  int imargin,
  statement stmt,
  bool braces_p,
  ...
  bool with_comments)
{
  ...
  text r= make_text(NIL);
  text temp;
  string i_comments = with_comments? statement_comments(stmt): string_undefined;
  ...
  // NN: Code added for C, because a statement can have its own declarations
  list dl = statement_declarations(stmt);

  // FI: consistency check - incompatible with unfolding and C parser...
  ifdebug(1) {
  ...
  if (!ENDP(dl) && prettyprint_language_is_c_p()) {
    ...
    if (declaration_statement_p(stmt)) {
      ...
      list il = declaration_statement_to_initializations(stmt);
      MERGE_TEXTS(local_var,
                  c_text_related_entities(module,dl,nmargin,sn,ppdl,il));
      //c_text_related_entities(module,dl,nmargin,sn,dl,il));
    }
    else {
  ...
  if (entity_return_label_p(statement_label(stmt))) {
      ...
      /* do not add a redundant RETURN before an END, unless
         ...
         in C */
      extern bool last_statement_p(statement);
...
text text_statement_enclosed(entity module,
    ...
    list * ppdl)
{
  return generic_text_statement_enclosed(
    module, imargin, stmt, braces_p, drop_continue_p, ppdl, true, true);
}
...
{
  list npdl = NIL;
  text t = text_statement_enclosed(module, imargin, stmt, braces_p, drop_continue_p, &npdl);
  gen_free_list(npdl);
  return t;
}
...
text text_statement(
    ...
    list * ppdl)
{
  return text_statement_enclosed(module, margin, stmt, true, true, ppdl);
}
...
text Text_Proper_Statement(
    entity module,
    int margin,
    statement stmt)
{
  list npdl = NIL;
  text t = generic_text_statement_enclosed(
    module, margin, stmt, true, true, &npdl, false, false);
  gen_free_list(npdl);
  return t;
}
 ...
 */
string proper_statement_to_string(statement cs)
{
  if (cs == NULL || statement_undefined_p(cs))
    return NULL;

  text t = Text_Proper_Statement(entity_undefined, 0, cs);
  string s = text_to_string_nl(t);
  free_text(t);
  return s;
}
...
void set_last_statement(statement s)
{
...

/* Reuse all declarations of the compilation unit, although some
 * may occur after the module definition.
 */
list initialize_previously_declared_entities(entity module)
{
  list pdl = NIL;
  if(c_module_p(module) && !compilation_unit_p(entity_user_name(module))) {
    entity cu = module_entity_to_compilation_unit_entity(module);
    
    if(entity_undefined_p(cu)) {
      pips_user_warning("A C source code is prettyprinted without using definitions placed in its compilation unit.\nThe resulting source code may not be meaningfull.\n");
      type t = entity_type(module);
      pdl = NIL;
      /* all types and derived entities used to define the formal
       * parameters must be predefined, or they are local to the
       * formal scope and useless.
       */
      pdl = type_supporting_entities(pdl, t);
    }
    else {
      code cuc = value_code(entity_initial(cu));
      pdl = gen_copy_seq(code_declarations(cuc));
    }
  }
  return pdl;
}
    ...
    statement stat)
{
  ...
  case is_language_c:
    ...
    if(!compilation_unit_p(entity_name(name))) {
      ...
      list pdl = initialize_previously_declared_entities(module);
	...
	MERGE_TEXTS(r,c_text_entities(module, l, INDENTATION, &pdl));
      ...
      /* Declarations linked to formal parameters are performed in a
	 different scope and do not have to be propagated. */
      gen_free_list(pdl);
    }
    break;
  default:
  ...
  if (stat != statement_undefined) {
    // Prevously declared entity list
    list pdl = initialize_previously_declared_entities(module);
...
static list words_cast(cast obj, int precedence, list * ppdl)
{
  ...
  pc = CHAIN_SWORD(pc, space_p? ") " : ")");
  pc = gen_nconc(pc, words_subexpression(exp, CAST_OPERATOR_PRECEDENCE, true, ppdl));
				   ...
				   list * ppdl)
{
  ...
  if (sizeofexpression_type_p(obj)) {
    ...
    if(derived_type_p(t)) {
      ...
      if(gen_in_list_p((void *) te, *ppdl)) {
	list pca = words_type(sizeofexpression_type(obj), ppdl, false);
	pc = gen_nconc(pc, pca);
      }
      else {
	/* The type must be fully declared: see struct15.c */
	list pct = c_words_simplified_entity(t, NIL, true, in_type_declaration, ppdl);
	pc = gen_nconc(pc, pct);
	*ppdl = gen_once((void *) te, *ppdl);
      }
    }
    else {
      list pca = words_type(sizeofexpression_type(obj), ppdl, false);
    ...
    pc = gen_nconc(pc, words_expression(sizeofexpression_expression(obj), ppdl));
  pc = CHAIN_SWORD(pc,")");
  return pc;
}
...
static list words_subscript(subscript s, list * ppdl)
{
  ...
  case is_language_fortran95: {
    ...
    pips_assert("We don't know how to prettyprint a subscript in Fortran95 "
		...
		allocatable_p );
    pc = gen_nconc(pc, words_expression(a, ppdl));
    if(!ENDP(lexp)) {
      pc = CHAIN_SWORD(pc,"(");
    }
    break;
...
static list words_application(application a, list * ppdl)
{
  ...
  pc = CHAIN_SWORD(pc,"(");
  pc = gen_nconc(pc, words_expression(f, ppdl));
  pc = CHAIN_SWORD(pc,")(");
  MAP(EXPRESSION,exp,
  {
    if (!first)
			 ...
			 bool is_recursive_p)
{
    ...
    if (!expression_undefined_p(forloop_initialization(obj)))
      pc = gen_nconc(pc, words_expression(forloop_initialization(obj), ppdl));
	...
	pc = gen_nconc(pc, words_expression(forloop_condition(obj), ppdl));
    }
    pc = CHAIN_SWORD(pc,C_STATEMENT_END_STRING);
    if (!expression_undefined_p(forloop_increment(obj)))
      pc = gen_nconc(pc, words_expression(forloop_increment(obj), ppdl));
    pc = CHAIN_SWORD(pc,!braces_p?")":") {");
    u = make_unformatted(strdup(label), n, margin, pc) ;
    ADD_SENTENCE_TO_TEXT(r, make_sentence(is_sentence_unformatted, u));
