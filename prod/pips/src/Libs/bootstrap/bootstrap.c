/*
  ...
  $Id: bootstrap.c 23065 2016-03-02 09:05:50Z coelho $
...
/* Function in flint/type_checker.c */
extern expression insert_cast(basic cast,
			      basic from,
			      expression exp,
			      type_context_p);
extern expression cast_constant(expression exp_constant, basic to_basic,
				type_context_p context);
extern bool check_loop_range(range, hash_table);
extern void type_loop_range(basic, range, type_context_p);
...
static type overloaded_to_long_integer_type(int n)
{
  type rt = MakeAnyScalarResult(is_basic_int, DEFAULT_LONG_INTEGER_TYPE_SIZE);
  functional ft = make_functional(NIL, rt);
  functional_parameters(ft) =
    make_parameter_list(n, MakeOverloadedParameter);
  type t = make_type(is_type_functional, ft);
...
static type integer_to_long_integer_type(int n)
{
  type rt = MakeAnyScalarResult(is_basic_int, DEFAULT_LONG_INTEGER_TYPE_SIZE);
  functional ft = make_functional(NIL, rt);
  functional_parameters(ft) =
...
static type long_integer_to_integer_type(int n)
{
  // type at = MakeAnyScalarResult(is_basic_int, DEFAULT_LONG_INTEGER_TYPE_SIZE);
  type rt = MakeIntegerResult();
  functional ft = make_functional(NIL, rt);
  functional_parameters(ft) =
...
/* Probably useless because of const_char_star_to_void_star_type() */
static type __attribute__ ((__unused__)) char_star_to_void_star_type(int n)
{
  functional ft = make_functional(NIL, MakeVoidPointerResult());
  functional_parameters(ft) =
...
static type char_pointer_to_double_type(int n __attribute__ ((unused)))
{
  ...
  //type cp = type_to_pointer_type(c); // FI: why is this commented out?
  type rt = MakeDoubleprecisionResult();
...
/* For system calls */
static type char_star_to_char_star_type(int n __attribute__ ((unused)))
{
  type at = MakeCharacterResult(); // Argument type
  type rt = MakeCharacterResult(); // Result type
  functional ft = make_functional(NIL, rt);
  functional_parameters(ft) =
    CONS(PARAMETER, make_parameter(at,
				   make_mode_value(), // not
						      // significant
						      // for void...
				   make_dummy_unknown()), NIL);
...
static type char_star_to_integer_type(int n __attribute__ ((unused)))
{
  type at = MakeCharacterResult();
  type rt = MakeIntegerResult();
  functional ft = make_functional(NIL, rt);
  functional_parameters(ft) =
    CONS(PARAMETER, make_parameter(at,
...
static type integer_to_char_star_type(int n __attribute__ ((unused)))
{
  type rt = MakeCharacterResult();
  type at = MakeIntegerResult();
  functional ft = make_functional(NIL, rt);
  functional_parameters(ft) =
    CONS(PARAMETER, make_parameter(at,
				   make_mode_value(), // not
...
static type const_char_star_to_integer_type(int n)
{
  type at = MakeCharacterResult();
  variable_qualifiers(type_variable(at))
    = CONS(QUALIFIER, make_qualifier_const(), NIL);
  type rt = MakeIntegerResult();
  functional ft = make_functional(NIL, rt);
  list pl = NIL;
  int i;
  for(i=0;i<n;i++) {
    pl = CONS(PARAMETER, make_parameter(copy_type(at),
				   make_mode_value(), // not
						      // significant
						      // for void...
				   make_dummy_unknown()), pl);
  }
  free_type(at);
  functional_parameters(ft) = pl;
  type t = make_type(is_type_functional, ft);
...
static type void_star_to_integer_type(int n)
{
  functional ft = make_functional(NIL, MakeIntegerResult());
  functional_parameters(ft) =
    make_parameter_list(n, MakeVoidPointerParameter);
  type t = make_type(is_type_functional, ft);
...
static type const_char_star_to_char_star_type(int n __attribute__ ((unused)))
{
  type at = MakeCharacterResult();
  variable_qualifiers(type_variable(at))
    = CONS(QUALIFIER, make_qualifier_const(), NIL);
...
/* Because of const_char_star_to_void_type() I guess */
static type __attribute__ ((__unused__)) char_star_to_void_type(int n __attribute__ ((unused)))
{
  type at = MakeCharacterResult();
...
static type void_to_char_star_type(int n __attribute__ ((unused)))
{
  type rt = MakeCharacterResult();
  type at = make_type_void(NIL);
  functional ft = make_functional(NIL, rt);
  functional_parameters(ft) =
    CONS(PARAMETER, make_parameter(at,
				   make_mode_value(), // not
...
type longlonginteger_to_overloaded_type(int n) /* MB */
{
  type t = type_undefined;
  functional ft = functional_undefined;
...
/* type char_star_to_void_type(int n) */
/* { */
/*   type t = type_undefined; */
/*   functional ft = functional_undefined; */
...
type void_to_overloaded_type(int n)
{
  type t = type_undefined;
  functional ft = functional_undefined;
...
void CreateIntrinsics( set module_list )
{
    ...
    static IntrinsicDescriptor IntrinsicTypeDescriptorTable[] =
    {
        ...
        {CTERMID_FUNCTION_NAME, 1, char_star_to_char_star_type, 0, 0},
        ...
        /* C IO system functions in man -S 2. The typing could be
	 * refined. 
	 *
	 * See also unistd.h below
	 */
	...
	/* unistd.h, in the header file order */
        ...
        {LSEEK_FUNCTION_NAME,     3,         default_intrinsic_type, 0, 0},
        {LSEEK64_FUNCTION_NAME,   3,         default_intrinsic_type, 0, 0},
        {C_CLOSE_FUNCTION_NAME,   1,         integer_to_integer_type,    0, 0},
        {C_READ_FUNCTION_NAME,    3,         default_intrinsic_type, 0, 0},
        {C_WRITE_FUNCTION_NAME,   3,         default_intrinsic_type, 0, 0}, /* returns ssize_t */
        {PREAD_FUNCTION_NAME,     4,         default_intrinsic_type, 0, 0},
        {PWRITE_FUNCTION_NAME,    4,         default_intrinsic_type, 0, 0},
        {PREAD64_FUNCTION_NAME,   4,         default_intrinsic_type, 0, 0},
        {PIPE_FUNCTION_NAME,      1,         overloaded_to_integer_type, 0, 0},
	// integer x integer -> integer
        {PIPE2_FUNCTION_NAME,     2,         overloaded_to_integer_type, 0, 0},
	// unsigned int -> unsigned int
        {SLEEP_FUNCTION_NAME,     2,         default_intrinsic_type, 0, 0},
        {UALARM_FUNCTION_NAME,    2,         default_intrinsic_type, 0, 0},
        {USLEEP_FUNCTION_NAME,    1,         overloaded_to_integer_type, 0, 0},
        {C_PAUSE_FUNCTION_NAME,   0,         void_to_integer_type, 0, 0},
        {CHOWN_FUNCTION_NAME,     3,         overloaded_to_integer_type, 0, 0},
        {FCHOWN_FUNCTION_NAME,    3,         overloaded_to_integer_type, 0, 0},
        {LCHOWN_FUNCTION_NAME,    3,         overloaded_to_integer_type, 0, 0},
        {FCHOWNAT_FUNCTION_NAME,  5,         overloaded_to_integer_type, 0, 0},
	// const_char_star_to_integer
        {CHDIR_FUNCTION_NAME,     1,         const_char_star_to_integer_type, 0, 0},
        {FCHDIR_FUNCTION_NAME,    1,         integer_to_integer_type, 0, 0},
        {GETCWD_FUNCTION_NAME,    2,         default_intrinsic_type, 0, 0},
        {GET_CURRENT_DIR_NAME_FUNCTION_NAME,0, void_to_char_star_type,0,0},
	// char_star_to_char_star
        {GETWD_FUNCTION_NAME,     1,         char_star_to_char_star_type, 0, 0},
        {DUP_FUNCTION_NAME,       1,         integer_to_integer_type, 0, 0},
        {DUP2_FUNCTION_NAME,      2,         integer_to_integer_type, 0, 0},
        {DUP3_FUNCTION_NAME,      3,         integer_to_integer_type, 0, 0},
        {EXECVE_FUNCTION_NAME,    3,         overloaded_to_integer_type, 0, 0},
        {FEXECVE_FUNCTION_NAME,   3,         overloaded_to_integer_type, 0, 0},
        {EXECV_FUNCTION_NAME,     2,         overloaded_to_integer_type, 0, 0},
        {EXECLE_FUNCTION_NAME,    INT_MAX,   overloaded_to_integer_type, 0, 0},
        {EXECL_FUNCTION_NAME,     INT_MAX,   overloaded_to_integer_type, 0, 0},
        {EXECVP_FUNCTION_NAME,    2,         overloaded_to_integer_type, 0, 0},
        {EXECLP_FUNCTION_NAME,    INT_MAX,   overloaded_to_integer_type, 0, 0},
        {EXECVPE_FUNCTION_NAME,   3,         overloaded_to_integer_type, 0, 0},
        {NICE_FUNCTION_NAME,      1,         integer_to_integer_type, 0, 0},
        {_EXIT_FUNCTION_NAME,     1,         integer_to_void_type, 0, 0},
        {PATHCONF_FUNCTION_NAME,  2 ,        overloaded_to_integer_type, 0, 0},
        {FPATHCONF_FUNCTION_NAME, 2,         integer_to_long_integer_type, 0, 0},
        {SYSCONF_FUNCTION_NAME,   1,         integer_to_long_integer_type, 0, 0},
        {CONFSTR_FUNCTION_NAME,   3,         default_intrinsic_type, 0, 0},
        {GETPID_FUNCTION_NAME,    0,         void_to_overloaded_type, 0, 0},
        {GETPPID_FUNCTION_NAME,   0,         void_to_overloaded_type, 0, 0},
        {GETPGRP_FUNCTION_NAME,   0,         void_to_overloaded_type, 0, 0},
        {__GETPGID_FUNCTION_NAME, 1,         default_intrinsic_type, 0, 0},
        {GETPGID_FUNCTION_NAME,   1,         default_intrinsic_type, 0, 0},
        {SETPGID_FUNCTION_NAME,   2,         overloaded_to_integer_type, 0, 0},
        {SETPGRP_FUNCTION_NAME,   0,         void_to_integer_type, 0, 0},
        {SETSID_FUNCTION_NAME,    0,         void_to_overloaded_type, 0, 0},
        {GETSID_FUNCTION_NAME,    1,         default_intrinsic_type, 0, 0},
        {GETUID_FUNCTION_NAME,    0,         void_to_overloaded_type, 0, 0},
        {GETEUID_FUNCTION_NAME,   0,         void_to_overloaded_type, 0, 0},
        {GETGID_FUNCTION_NAME,    0,         void_to_overloaded_type, 0, 0},
        {GETEGID_FUNCTION_NAME,   0,         void_to_overloaded_type, 0, 0},
        {GETPGROUPS_FUNCTION_NAME,2,         overloaded_to_integer_type, 0, 0},
        {GROUP_MEMBER_FUNCTION_NAME,1,       overloaded_to_integer_type, 0, 0},
        {SETUID_FUNCTION_NAME,    1,         overloaded_to_integer_type, 0, 0},
        {SETREUID_FUNCTION_NAME,  1,         overloaded_to_integer_type, 0, 0},
        {SETEUID_FUNCTION_NAME,   1,         overloaded_to_integer_type, 0, 0},
        {SETGID_FUNCTION_NAME,    1,         overloaded_to_integer_type, 0, 0},
        {SETREGID_FUNCTION_NAME,  2,         overloaded_to_integer_type, 0, 0},
        {SETEGID_FUNCTION_NAME,   1,         overloaded_to_integer_type, 0, 0},
        {GETRESUID_FUNCTION_NAME, 3,         overloaded_to_integer_type, 0, 0},
        {GETRESGID_FUNCTION_NAME, 3,         overloaded_to_integer_type, 0, 0},
        {SETRESUID_FUNCTION_NAME, 3,         overloaded_to_integer_type, 0, 0},
        {SETRESGID_FUNCTION_NAME, 3,         overloaded_to_integer_type, 0, 0},
        {FORK_FUNCTION_NAME,      0,         void_to_overloaded_type, 0, 0},
        {VFORK_FUNCTION_NAME,     0,         void_to_overloaded_type, 0, 0},
        {TTYNAME_FUNCTION_NAME,   1,         integer_to_char_star_type, 0, 0},
        {TTYNAME_R_FUNCTION_NAME, 3,         overloaded_to_integer_type, 0, 0},
        {ISATTY_FUNCTION_NAME,    1,         integer_to_integer_type, 0, 0},
        {TTYSLOT_FUNCTION_NAME,   0,         void_to_integer_type, 0, 0},
	{LINK_FUNCTION_NAME,      2,         const_char_star_to_integer_type, 0, 0},
        {LINKAT_FUNCTION_NAME,    5,         overloaded_to_integer_type, 0, 0},
        {SYMLINK_FUNCTION_NAME,   2,         overloaded_to_integer_type, 0, 0},
        {READLINK_FUNCTION_NAME,  3,         default_intrinsic_type, 0, 0},
        {SYMLINKAT_FUNCTION_NAME, 3,         overloaded_to_integer_type, 0, 0},
        {READLINKAT_FUNCTION_NAME,4,         default_intrinsic_type, 0, 0},
	// const char * -> integer
        {UNLINK_FUNCTION_NAME,    1,         const_char_star_to_integer_type, 0, 0},
        {UNLINKAT_FUNCTION_NAME,  3,         overloaded_to_integer_type, 0, 0},
        {RMDIR_FUNCTION_NAME,     1,         const_char_star_to_integer_type, 0, 0},
        {TCGETPGRP_FUNCTION_NAME, 1,         integer_to_overloaded_type, 0, 0},
        {TCSETPGRP_FUNCTION_NAME, 2,         overloaded_to_integer_type, 0, 0},
        {GETLOGIN_FUNCTION_NAME,  0,         void_to_char_star_type, 0, 0},
        {GETLOGIN_R_FUNCTION_NAME,2,         overloaded_to_integer_type, 0, 0},
        {SETLOGIN_FUNCTION_NAME,  1,         char_star_to_integer_type, 0, 0},
        {GETHOSTNAME_FUNCTION_NAME,2,        overloaded_to_integer_type, 0, 0},
        {SETHOSTNAME_FUNCTION_NAME,2,        overloaded_to_integer_type, 0, 0},
        {SETHOSTID_FUNCTION_NAME,  1,        long_integer_to_integer_type, 0, 0},
        {GETDOMAINNAME_FUNCTION_NAME,2,      overloaded_to_integer_type, 0, 0},
        {SETDOMAINNAME_FUNCTION_NAME,2,      overloaded_to_integer_type, 0, 0},
        {VHANGUP_FUNCTION_NAME,   0,         void_to_integer_type, 0, 0},
	// const_char_star_to_integer
        {REVOKE_FUNCTION_NAME,    1,         const_char_star_to_integer_type, 0, 0},
        {PROFIL_FUNCTION_NAME,    4,         overloaded_to_integer_type, 0, 0},
	// const_char_star_to_integer
        {ACCT_FUNCTION_NAME,      1,         const_char_star_to_integer_type, 0, 0},
        {GETUSERSHELL_FUNCTION_NAME,0,       void_to_char_star_type, 0, 0},
        {ENDUSERSHELL_FUNCTION_NAME,0,       void_to_void_type, 0, 0},
        {SETUSERSHELL_FUNCTION_NAME,0,       void_to_void_type, 0, 0},
        {DAEMON_FUNCTION_NAME,    2,         integer_to_integer_type, 0, 0},
        {CHROOT_FUNCTION_NAME,    1,         const_char_star_to_integer_type, 0, 0},
        {GETPASS_FUNCTION_NAME,   2,         const_char_star_to_char_star_type, 0, 0},
        {FSYNC_FUNCTION_NAME,     1,         integer_to_integer_type, 0, 0},
        {SYNCFS_FUNCTION_NAME,    1,         integer_to_integer_type, 0, 0},
	// void_to_long_integer
        {GETHOSTID_FUNCTION_NAME, 0,         void_to_overloaded_type, 0, 0},
        {GETPAGESIZE_FUNCTION_NAME,0,        void_to_integer_type, 0, 0},
        {GETDTABLESIZE_FUNCTION_NAME,0,      void_to_integer_type, 0, 0},
        {TRUNCATE_FUNCTION_NAME,  2,         overloaded_to_integer_type, 0, 0},
        {TRUNCATE64_FUNCTION_NAME,2,         overloaded_to_integer_type, 0, 0},
        {FTRUNCATE_FUNCTION_NAME, 2,         overloaded_to_integer_type, 0, 0},
        {FTRUNCATE64_FUNCTION_NAME,2,        overloaded_to_integer_type, 0, 0},
        {BRK_FUNCTION_NAME,       1,         void_star_to_integer_type, 0, 0},
        {SBRK_FUNCTION_NAME,      1,         default_intrinsic_type, 0, 0},
        {SYSCALL_FUNCTION_NAME,   INT_MAX,   overloaded_to_long_integer_type, 0, 0},
        {LOCKF_FUNCTION_NAME,     3,         overloaded_to_integer_type, 0, 0},
        {LOCKF64_FUNCTION_NAME,   3,         overloaded_to_integer_type, 0, 0},
        {FDATASYNC_FUNCTION_NAME, 2,         integer_to_integer_type, 0, 0},
        {CRYPT_FUNCTION_NAME,     2,         const_char_star_to_char_star_type, 0, 0},
        {ENCRYPT_FUNCTION_NAME,   2,         overloaded_to_void_type, 0, 0},
        {SWAB_FUNCTION_NAME,      3,         overloaded_to_void_type, 0, 0},
 ...
 */
	...
	// Also in unistd.h
        ...
        {MKTIME_FUNCTION_NAME,             1, default_intrinsic_type, 0, 0},
        {STRFTIME_FUNCTION_NAME,           3, default_intrinsic_type, 0, 0},
        {STRPTIME_FUNCTION_NAME,           3, default_intrinsic_type, 0, 0},
        {STRFTIME_L_FUNCTION_NAME,         4, default_intrinsic_type, 0, 0},
        {STRPTIME_L_FUNCTION_NAME,         4, default_intrinsic_type, 0, 0},	
        {GMTIME_FUNCTION_NAME,             1, default_intrinsic_type, 0, 0},	
        {LOCALTIME_FUNCTION_NAME,          1, default_intrinsic_type, 0, 0},
        {GMTIME_R_FUNCTION_NAME,           2, default_intrinsic_type, 0, 0},	
        {LOCALTIME_R_FUNCTION_NAME,        2, default_intrinsic_type, 0, 0},
        {ASCTIME_FUNCTION_NAME,            1, default_intrinsic_type, 0, 0},	
        {CTIME_FUNCTION_NAME,              1, default_intrinsic_type, 0, 0},	
        {ASCTIME_R_FUNCTION_NAME,          2, default_intrinsic_type, 0, 0},	
        {CTIME_R_FUNCTION_NAME,            2, default_intrinsic_type, 0, 0},	
        {TZSET_FUNCTION_NAME,              0, void_to_void_type, 0, 0},	
        {STIME_FUNCTION_NAME,              1, overloaded_to_integer_type, 0, 0},
        {TIMEGM_FUNCTION_NAME,             1, default_intrinsic_type, 0, 0},	
        {TIMELOCAL_FUNCTION_NAME,          1, default_intrinsic_type, 0, 0},	
        {DYSIZE_FUNCTION_NAME,             2, integer_to_integer_type, 0, 0},
        {NANOSLEEP_FUNCTION_NAME,          2, default_intrinsic_type, 0, 0},
        {CLOCK_GETRES_FUNCTION_NAME,       2, overloaded_to_integer_type, 0, 0},
        {CLOCK_GETTIME_FUNCTION_NAME,      2, overloaded_to_integer_type, 0, 0}, // BSD-GNU
        {CLOCK_SETTIME_FUNCTION_NAME,      2, overloaded_to_integer_type, 0, 0},
        {CLOCK_NANOSLEEP_FUNCTION_NAME,    3, overloaded_to_integer_type, 0, 0},
        {CLOCK_GETCPULOCKID_FUNCTION_NAME, 2, overloaded_to_integer_type, 0, 0},
        {TIMER_CREATE_FUNCTION_NAME,       3, overloaded_to_integer_type, 0, 0},
        {TIMER_DELETE_FUNCTION_NAME,       1, overloaded_to_integer_type, 0, 0},
        {TIMER_SETTIME_FUNCTION_NAME,      3, overloaded_to_integer_type, 0, 0},
        {TIMER_GETTIME_FUNCTION_NAME,      2, overloaded_to_integer_type, 0, 0},
        {TIMER_GETOVERRUN_FUNCTION_NAME,   1, overloaded_to_integer_type, 0, 0},
        {GETDATE_FUNCTION_NAME,            1, default_intrinsic_type, 0, 0},
        {GETDATE_R_FUNCTION_NAME,          2, overloaded_to_integer_type, 0, 0},
...
bool bootstrap(const string workspace)
{
