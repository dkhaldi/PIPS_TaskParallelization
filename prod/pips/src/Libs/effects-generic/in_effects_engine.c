/*
  ...
  $Id: in_effects_engine.c 23065 2016-03-02 09:05:50Z coelho $
...
// for add_intermediate_value() and entity_to_intermediate_value()
...
static bool in_effects_stmt_filter(
  statement s, _UNUSED_ in_effects_context *ctxt)
{
...
static void in_effects_of_statement(
  statement s, _UNUSED_ in_effects_context *ctxt)
{
 ...
 */
static list in_effects_of_declarations(
  list lin_after_decls, list l_decl,
...
static void in_effects_of_loop(loop l, in_effects_context *ctxt)
{
  ...
  if (!ENDP(global_in))
  {
    ...
    // WIP
    (*loop_range_in_effect_func)(l, &global_in, &global_write,
                                 r_w_combinable_p, &i, r);
  }
...
static void
in_effects_of_unstructured(unstructured u, _UNUSED_ in_effects_context *ctxt)
{
  statement current_stat = effects_private_current_stmt_head();
  list blocs = NIL ;
  control ct;
