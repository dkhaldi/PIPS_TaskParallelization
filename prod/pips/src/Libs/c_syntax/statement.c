...
#include "misc.h"
#include "text-util.h"
#include "prettyprint.h" // for fprint_expression in an error message
                         // and for words_expression() really used
                         // Maybe, the same stuff could be programmed otherwise
 ...
 *
 * Before the code can be generated, expression e must be statically
 * evaluated, transformer into a character string, and this character
 * string must be made compatible with C constraints for labels.
 */
...
/* The control list is hidden as arguments to the call to CONTROL in
 * the declaration statement s.
 *
 * It is made of 0 and 1 constant expressions.
 *
 * The first element tells the prettyprinter if the "extern" keyword
 * must be printed out.
 *
 * The next expressions tells the prettyprinter which variables should
 * be initialized by this statement.
 *
 * The control list for the initializations is built by the C parser, cyacc.y.
 *
 * This is only useful for global variables which can be declared
 * multiple times in multiple files and in multiple ways. See
 * Prettyprint/double_declaration01-14.c.
 */
list add_prettyprint_control_list_to_declaration_statement
(statement s, list initialization_expressions)
{
...
/*   list cel = call_arguments(c); */
/*   if(ENDP(cel)) { */
/*     cel = CONS(EXPRESSION, int_to_expression(1), NIL); */
/*   } */
/*   else { */
/*     expression ce = EXPRESSION(CAR(cel)); */
/*     int extern_p = integer_constant_expression_value(ce); */
/*     if(!extern_p) { */
/*       EXPRESSION_(CAR(cel)) = int_to_expression(1); */
/*       free_expression(ce); */
/*     } */
/*   } */
/*   call_arguments(c) = cel; */
/* } */

/* Force declaration statement s to be a dummy derived entity
 * declaration such as struct s;
 */
