/*
  ...
  $Id: declaration.c 23065 2016-03-02 09:05:50Z coelho $
...
/* This function should not be used outside of the syntax library
 * because it depends on ParserError().
 *
...

/*
 * COMMONs are handled as global objects. They may pre-exist when the
 * current module is parsed. They may also be declared in more than
 * one statement. So we need to keep track of the current size of each
 * common encountered in the current module with a mapping from entity
 * to integer, common_size_map.
 *
 * At the end of the declaration phase, common sizes can be either set
 * if yet unknown, or compared for equality with a pre-defined size.
 */
static hash_table common_size_map = hash_table_undefined;

/* These tests are needed to check area consistency when dumping or
...
/* 
 * This function adds a variable v to a common block c. v's storage
 * must be undefined.
 *
 * c's size used to be indirectly updated by CurrentOffsetOfArea() but
 * this is meaningless because v's type and dimensions are
 * unknown. The layouts of commons are updated later by
 * update_common_sizes() called from EndOfProcedure().
 */ 
void 
AddVariableToCommon(entity c, entity v)
{
 ...
 *
 * Note FI: this function is called too early by the Fortran parser
 * because a DIMENSION or a Type statement can modify both the basic
 * type and the dimensions of variable v.
 *
 ...
 * common_size_map...
 *
 * Note: the useful part of this function has been named
 * current_offset_of_area(0 and moved into ri-util. This function
 * seems about obsolete and never used with Fortran common. Most of
 * its call sites in syntax have been commented out.
 */
...
/* This function computes the Fortran implicit type of entity e. The first
...
/* FI: I do not understand the naming here, or the parameter. The name
 * of the new variable must be a global name, whereas the name of the
 * area is a local name. And the area does not have to be the stack
 * area...
 */
void add_entity_to_declarations (string name, string area_name, enum basic_utype tag,
								 void* val) {
