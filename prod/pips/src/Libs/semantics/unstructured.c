				     ...
				     bool postcondition_p)
{
  ...
  if(!meaningless_control_p(c)) {
	...
	post = transformer_empty();
      ...
      semantics_user_warning("After restructuration, unexpected unreachable node:%s",
			statement_identification(s));
      ...
      semantics_user_warning("After restructuration (?),"
...
static void unreachable_node_to_transformer(control c)
{
  ...
  if(statement_ordering(s)==STATEMENT_ORDERING_UNDEFINED) {
    semantics_user_warning("Improper control restructuration, statement with no ordering:%s"
