/*
  ...
  $Id: alias_propagation.c 23065 2016-03-02 09:05:50Z coelho $
...
#include "callgraph.h" // for module_is_called_by_main_program_p
						     ...
						     list l_actual,bool equiv)
{
  if (equiv)
    ...
    {
        string istr = int2a(unique_section_number++);
...
static bool add_aliases_for_current_call_site(call c)
{
  if(call_function(c) == current_mod)
      ...
      ifdebug(2)
	{
      ...
      l_traversed = NIL;
      FOREACH(EXPRESSION,actual_arg, l)
      {
	i++;
	if (expression_reference_p(actual_arg))
	  {
		...
		if ((int)gen_length(l_traversed)>j) equiv = true;
