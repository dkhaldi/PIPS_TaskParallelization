#! /usr/bin/perl -w
#
# $Id: pips_validation_summary.pl 23150 2016-07-14 18:44:38Z coelho $
#
...
my $others = 'skipped|multi-script|multi-source|nofilter|broken-directory|' .
             'missing|empty-test';
...
my $warned = $n{skipped} + $n{nofilter} +  $n{'multi-script'} + $n{missing} +
    ...
    $n{'empty-test'};
my $ignored = $n{skipped} + $n{bug} + $n{later} + $n{slow};
my $total = $n{passed} + ${not_passed} + ${ignored} + ${cannot_execute};
...
print
  "number of tests run: $count/$total\n" .
  " * passed: $n{passed}\n" .
  ...
  " * ignored: $ignored ($n{skipped} skipped, $n{bug} bug, " .
               "$n{later} later, $n{slow} slow)\n";
...
print "directory", " " x 21,
      "cases  runs  bads (F+C+T+N+O|K+B+L+S) changes...\n";
for my $dir (sort keys %d)
{
  my $failures =
    $d{$dir}{failed} + $d{$dir}{changed} + $d{$dir}{timeout} +
    $d{$dir}{orphan} + $d{$dir}{noref};
  # runs may be null if all tests are kept out
  my $runs = $d{$dir}{passed} + $failures;
  my $cases = $runs + $d{$dir}{keptout} + $d{$dir}{bug} +
                      $d{$dir}{later}   + $d{$dir}{slow};

  printf "%-30s %4d  %4d  %4d", $dir, $cases, $runs, $failures;

  # show some details
  if ($failures != 0 or $runs != $cases or
