# $Id: libraries.make 22640 2015-07-18 18:31:38Z coelho $
#
...
ifdef PIPS_SPECIAL
# compile a specialized version of pips
spec.libs = $(PIPS_ROOT)/src/Documentation/pipsmake/$(PIPS_SPECIAL).libs
# does it need to be more resilient?
pipslibs.libs	= $(shell cat $(spec.libs))
else
pipslibs.libs	= \
	top-level pipsmake static_controlize paf-util wp65 hpfc hyperplane \
	to_begin_with instrumentation locality expressions transformations \
	syntheses accel-util hwac movements bootstrap callgraph icfg chains \
	complexity conversion modeling prettyprint atomizer \
	pointer_values points-to alias-classes \
	effects-simple effects-convex effects-generic \
	comp_sections semantics continuation rice ricedg \
	transformer preprocessor effects-util step  \
	reductions flint sac safescale phrase \
	gpu scalopes pocc-interface taskify rstream_interface \
	regions_to_loops task_parallelization syntax c_syntax \
	workspace-util control preprocessor pipsdbm ri-util naming \
	text-util properties misc newgen
endif # PIPS_SPECIAL
